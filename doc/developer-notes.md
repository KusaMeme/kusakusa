...
- [Development guidelines](#development-guidelines)
    - [General Kusa Core](#general-kusa-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Kusa Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/kusachain/kusa/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/kusad -printtoconsole
```
...
Kusa Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Kusa Core, where everyone uses
...
pay attention to for reviewers of Kusa Core code.
...
- Avoid using locale dependent functions if possible. You can use the provided
  ...
  - These functions are known to be locale dependent:
    ...
    `fputws`, `kusanf`, `fwprintf`, `getdate`, `getwc`, `getwchar`, `isalnum`,
    ...
    `vdprintf`, `versionsort`, `vfprintf`, `vkusanf`, `vfwprintf`, `vprintf`,
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Kusa Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Kusa Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `kusa-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `kusa-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Kusa Core, the wallet was always
