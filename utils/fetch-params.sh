...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/KusaParams"
else
    PARAMS_DIR="$HOME/.kusa-params"
fi
...
DOWNLOAD_URL="https://github.com/kusachain/kusa/releases/tag/1.2.15.3"
...
IPKUSMD="$(command -v ipfs || echo '')"
CURLCMD="$(command -v curl || echo '')"

# fetch methods can be disabled with KUS_DISABLE_SOMETHING=1
KUS_DISABLE_WGET="${KUS_DISABLE_WGET:-}"
#KUS_DISABLE_IPFS="${KUS_DISABLE_IPFS:-}"
KUS_DISABLE_CURL="${KUS_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$KUS_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPKUSMD" ] || ! [ -z "$KUS_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$KUS_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Kusa parameters!
...
This script will fetch the Kusa SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Kusa SNARK parameters. Note that it is
