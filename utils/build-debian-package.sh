...
PACKAGE_VERSION=$($SRC_PATH/src/kusad --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/kusad $DEB_BIN
cp $SRC_PATH/src/kusa-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/kusa-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/kusad.1 $DEB_MAN
cp $SRC_DOC/man/kusa-cli.1 $DEB_MAN
cp $SRC_DOC/man/kusa-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/kusad.bash-completion $DEB_CMP/kusad
cp $SRC_PATH/contrib/kusa-cli.bash-completion $DEB_CMP/kusa-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/kusad.1
gzip --best -n $DEB_MAN/kusa-cli.1
gzip --best -n $DEB_MAN/kusa-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/kusad $DEB_BIN/kusa-cli
