...
const char * const BITCOIN_CONF_FILENAME = "kusa.conf";
const char * const BITCOIN_PID_FILENAME = "kusad.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "kusad -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KusaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KusaCore
    // Mac: ~/Library/Application Support/KusaCore
    // Unix: ~/.kusacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KusaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KusaCore";
#else
    // Unix
    return pathRet / ".kusacore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty kusa.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Kusa Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2023, nEndYear) + "The Kusa Core developers";
    }
