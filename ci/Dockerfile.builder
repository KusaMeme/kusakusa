...
# kusa_hash
RUN git clone https://github.com/kusachain/kusa_hash
RUN cd kusa_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} kusa
RUN useradd -u ${USER_ID} -g kusa -s /bin/bash -m -d /kusa kusa
...
RUN mkdir /kusa-src && \
  ...
  chown $USER_ID:$GROUP_ID /kusa-src && \
