...
class BitcoinTestFramework():
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave kusads and test.* datadir on exit or error")
        ...
        parser.add_option("--srcdir", dest="srcdir", default=os.path.abspath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"),
                          help="Source directory containing kusad/kusa-cli (default: %default)")
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use kusa-cli instead of RPC for all commands")
        parser.add_option("--kusad-arg", dest="dashd_extra_args", default=[], type='string', action='append',
                          help="Pass extra args to all kusad instances")
        ...
        self.options.bitcoind = os.getenv("BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/kusad' + config["environment"]["EXEEXT"])
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/kusa-cli' + config["environment"]["EXEEXT"])
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: kusads were not stopped and may still be running")
    ...
    def stop_node(self, i, wait=0):
        """Stop a kusad test node"""
        self.nodes[i].stop_node(wait=wait)
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self, wait=0):
        """Stop multiple kusad test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as kusad's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self, extra_args=None, stderr=None):
        ...
        if create_cache:
            ...
            # Create cache directories, run kusads:
            self.set_genesis_mocktime()
            for i in range(MAX_NODES):
...
class KusaTestFramework(BitcoinTestFramework):
    def set_kusa_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False):
...
def skip_if_no_bitcoind_zmq(test_instance):
    """Skip the running test if kusad has not been compiled with zmq support."""
    config = configparser.ConfigParser()
    config.read_file(open(test_instance.options.configfile))

    if not config["components"].getboolean("ENABLE_ZMQ"):
        raise SkipTest("kusad has not been built with zmq enabled.")
